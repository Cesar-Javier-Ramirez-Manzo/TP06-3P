...
$mysqli = new mysqli('localhost:3306', 'root', '', 'cpremier');
...
$sql = "SELECT idtrabajador, nombre, tarifaHr, oficio, idsupv FROM trabajador WHERE idtrabajador = $idt";
...
$sql = "SELECT idtrabajador, nombre FROM trabajador ORDER BY 1;";
if (!$resultado = $mysqli->query($sql)) {
    echo "Lo sentimos, este sitio web está experimentando problemas.";
	echo "</body></html>";
    exit;
...
while ($trabajador = $resultado->fetch_assoc()) {
    echo "<li><a href='./testmysql_cpremier.php?idt=" . $trabajador['idtrabajador'] . "'>\n";
    echo $trabajador['idtrabajador'] . ' - ' . $trabajador['nombre'];
    echo "</a></li>\n";
}
echo "</ul>\n";
echo "</body></html>";
